...
st.set_page_config(page_title="CareLog", layout="centered", page_icon="🌸")

# 탭 생성
tab1, tab2, tab3 = st.tabs(["알러지 검사", "챗봇", "걸음수 측정"])
...
# 탭 3: 걸음수 측정
with tab3:
    st.markdown("## 🏃‍♂️ CareLog 걸음수 측정")
    st.markdown("일일 걸음수를 기록하고 건강 목표를 달성하세요!")

    # 세션 상태 초기화
    if "step_count" not in st.session_state:
        st.session_state.step_count = 0
    if "daily_goal" not in st.session_state:
        st.session_state.daily_goal = 10000

    # 목표 설정
    st.markdown("### 🎯 일일 목표 설정")
    new_goal = st.number_input("일일 걸음 목표", min_value=1000, max_value=50000, value=st.session_state.daily_goal, step=1000)
    if st.button("목표 설정"):
        st.session_state.daily_goal = new_goal
        st.success(f"목표가 {new_goal}걸음으로 설정되었습니다!")

    # 현재 걸음수 표시
    st.markdown("### 📊 현재 걸음수")
    progress = min(st.session_state.step_count / st.session_state.daily_goal, 1.0)
    st.progress(progress)
    st.markdown(f"**{st.session_state.step_count} / {st.session_state.daily_goal} 걸음** ({progress*100:.1f}%)")

    if st.session_state.step_count >= st.session_state.daily_goal:
        st.balloons()
    ...
    with col1:
        if st.button("걷기 (+100걸음)"):
            st.session_state.step_count += 100
            st.rerun()
    with col2:
        if st.button("달리기 (+500걸음)"):
            st.session_state.step_count += 500
            st.rerun()
    with col3:
        if st.button("운동 (+1000걸음)"):
            st.session_state.step_count += 1000
            st.rerun()

    # 수동 입력
    manual_steps = st.number_input("직접 걸음수 입력", min_value=0, step=10)
    if st.button("추가"):
        st.session_state.step_count += manual_steps
        st.rerun()

    # 리셋
    if st.button("하루 리셋"):
        st.session_state.step_count = 0
        st.info("걸음수가 리셋되었습니다.")
